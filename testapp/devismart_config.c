...
{
	bool isStringType	= (tok->type == JSMN_STRING);
	bool isSameLength	= ((int) strlen(s) == tok->end - tok->start);
	bool isEqual		= strncmp(json + tok->start, s, tok->end - tok->start) == 0;
	return isStringType && isSameLength && isEqual;
}
...
static osdg_result_t devismart_request_configuration(osdg_connection_t connection)
{
  ...
  printf("Sending configuration request: %s\n", json);
...
{
    // Received data is a JSON and it looks like this:
	/* {
	 	"housePeerId":		"xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx",
		"houseName":		"M41",
		"houseEditUsers":	false,
		"rooms": [
			{ "zone": "None",	"sortOrder": 0,	"roomName": "Bad"			},
			{ "zone": "Living",	"sortOrder": 0,	"roomName": "Wohnzimmer"	},
			{ "zone": "Living",	"sortOrder": 1,	"roomName": "WC"			},
			{ "zone": "Living",	"sortOrder": 2,	"roomName": "Küche"			}
			{ "zone": "Living",	"sortOrder": 3,	"roomName": "Schlafzimmer"	},
			{ "zone": "Living",	"sortOrder": 4,	"roomName": "Büro"			},
			{ "zone": "Living",	"sortOrder": 5,	"roomName": "Kaminzimmer"	},
		]
	} */
	printf("parse_config_data(): %.*s\n", size, json);
	...
	for (i = 1; i < r; i++) {
		...
		if (jsoneq(json, &t[i], "housePeerId")) {
			housePeer = malloc(len + 1);
			if (housePeer) {
				memcpy(housePeer, json + tok->start, len);
				housePeer[len] = 0;
			}

		} else if (jsoneq(json, &t[i], "houseName")) {
			houseName = malloc(len + 1);
			if (houseName) {
				memcpy(houseName, json + tok->start, len);
				houseName[len] = 0;
			}
		}
    }

	if (housePeer != NULL && houseName != NULL) {
		osdg_key_t housePeerId;
		if (! osdg_hex_to_bin(housePeerId, sizeof(housePeerId), housePeer, sizeof(osdg_key_t) * 2, NULL, NULL, NULL)) {
			add_pairing(housePeerId, houseName);
		} else {
			if (houseName == NULL) {
				houseName = "";
			}
...
static void devismart_config_status_changed(osdg_connection_t conn, enum osdg_connection_state status)
{
	...
	if (status == osdg_connected) {
		osdg_result_t err = devismart_request_configuration(conn);
        if (err) {
